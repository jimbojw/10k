...
var b=j.defaultView,a=[],c=0,d="10score",f=/\s+/g,g,e,n,s,q=j.getElementsByTagName("p");g=0;for(e=q.length;g<e;g++)i(q[g]);q=j.getElementsByTagName("div");var u,p=/<(a|blockquote|dl|div|img|ol|p|pre|table|ul|br)/i;g=0;for(e=q.length;g<e;g++){u=q[g];p.test(u.innerHTML)&&i(u)}p=null;var v;for(g=0;g<c;g++){e=a[g];q=e.getElementsByTagName("a");n=v=0;for(s=q.length;n<s;n++){u=q[n];v+=(u.innerText||u.textContent).length}e[d]=score=e[d]*(1-v/(e.innerText||e.textContent).length);if(!p||score>p[d])p=e}p=p||
j.body;q=j.getSelection()||"";v="h1,h2,h3,h4,h5,h6,pre,code".split(",");u=[];var y,o;g=0;for(e=v.length;g<e;g++){y=j.getElementsByTagName(v[g]);n=0;for(s=y.length;n<s;n++){o=y[n];k(o)&&u.push(o.textContent)}}n=[];p=[p];var w;s=/[a-z][a-z][a-z]/i;for(v=/button|link|noscript|script|style/i;e=p.shift();){y=e.childNodes;g=0;for(e=y.length;g<e;g++){o=y[g];w=o.nodeType;if(w===1&&!v.test(o.tagName)&&k(o))p[p.length]=o;else if(w===3){text=o.nodeValue;if(s.test(text))n[n.length]=text}}}r["10kse"].iframe.contentWindow.postMessage(JSON.stringify({url:j.location.href,
selection:q,title:j.title,priority:u.join(" "),content:n.join(" ")}),"*")}})(window["10kse"]);
...
(function(l,r,j){var m=r("<div></div>");l.highlight=function(h,k,i){if(i===j)i=true;h=m.text(h).html();var b=h.toLowerCase(),a={},c={},d=[],f,g,e,n,s,q;f=0;for(g=k.length;f<g;f++){e=k[f].toLowerCase();n=e.length;if(n>2&&!a[e]){a[e]=true;for(q=s=0;s<3;){q=b.indexOf(e,q);if(q===-1)break;var u=h.charAt(q-1).toLowerCase(),p=h.charAt(q+n).toLowerCase();if((u<"a"||u>"z")&&(p<"a"||p>"z")){s++;d[d.length]=q;c[q]=e}q+=n}}}d.sort(l.asc);k=[];b=0;n=d.length;for(f=q=0;b<200&&f<=n;){a=d[f];e=f<n?h.substr(q,a-
q):h.substr(q);g=e.length;if(i)if(f===n){if(g>50)e=e.substr(0,46)+" ..."}else if(f===0){if(g>50)e="... "+e.substr(g-50-4)}else if(g>100)e=e.substr(0,47)+" ... "+e.substr(g-50-3);k[k.length]=e;b+=e.length;if(f<n){e=c[a];k[k.length]="<b>"+h.substr(a,e.length)+"</b>";b+=e.length;q=a+e.length}f++}if(b>=200&&f<=n)k[k.length]=" ...";return k.join("")}})(window["10kse"],jQuery);
...
(function(l,r,j){function m(f){function g(){y(v);v=null;if(p){var o=p.text(),w=n.val(),t=w.lastIndexOf(" ")+1;n.get(0).value=u=w.substr(0,t)+o;p.removeClass(c);p=null}s.hide()}function e(){if(q.find("li").length){var o=n.offset();s.css({top:o.top+f.offsetHeight,left:o.left,width:f.offsetWidth});s.show()}}var n=j(f),s=j('<div class="auto"><ul></ul></div>').css({display:"none"}).appendTo(document.body),q=s.find("ul"),u="",p=null,v,y=l.clearTimeout;n.bind("change keyup",function(o){var w=n.val();o=o.which;
if(w!==u){u=w;o=w.lastIndexOf(" ");var t=w.substr(o<0?0:o).toLowerCase();if(t.length){v&&y(v);v=l.setTimeout(function(){y(v);v=null;var x=d(t);if(x&&x.length){q.empty();p=null;for(var z=0,A=x.length;z<A;z++)j("<li><span></span></li>").appendTo(q).find("span").html(x[z].split(t,2).join("<b>"+t+"</b>"));e()}else s.hide()},100)}else s.hide()}else if(o===h||o===i){if(p)if(o===h){w=p.prev();if(w.length){p.removeClass(c);p=w.addClass(c)}}else{w=p.next();if(w.length){p.removeClass(c);p=w.addClass(c)}}else p=
q.find("li").eq(0).addClass(c);e()}else if(o===b||o===k)g();else o===a&&s.hide()}).parent("form").submit(g);q.mouseover(function(o){o=j(o.target);o=o.is("li")?o:o.parent("li");if(o=o.length?o:null){p&&p.get(0)!==o.get(0)&&p.removeClass(c);p=o.addClass(c)}}).click(g)}var h=38,k=39,i=40,b=13,a=27,c="selected",d=r.suggest;j(".search input").each(function(){m(this)});r.autocomplete=m})(window,window["10kse"],jQuery);
...
e),-1)]];f={};a=0;for(c=e.length;a<c;a++){var n=e[a];d=n[0];n=n[1];for(g in n){f[g]||(f[g]=0);f[g]+=d*n[g]}}e=[];a=0;d={};var s;for(g in f){s=f[g];n=d[s]||(d[s]=[]);n[n.length]=g;e[a++]=s}e.sort(j.asc);e.reverse();var q=j.highlight,u=null;k.empty();a=0;for(c=e.length;a<c;a++){s=e[a];if(s!==u){u=s;n=d[s];s=0;for(var p=n.length;s<p;s++){g=n[s];var v=h("ID-"+g),y=h("URL-"+v),o=y.text;k.append(m("<dt><a></a></dt>").find("a").attr("href",v).attr("title",y.title).html(q(y.title,b,false)).end(),m("<dd><p></p></dd>").find("p").html(q(o,
