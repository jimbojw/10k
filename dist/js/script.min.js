...
(function(p,r){var k=p.localStorage,o,h;r.suggest=function(m){m=m.replace(/^\s+|\s$/g,"").toLowerCase();if(!m||!k.length)return[];var i;i=k.length;if(i!==h){h=i;o=[];for(var a=i=0;a<h;a++){var b=k.key(a);if(b.indexOf("W-")===0)o[i++]=b.substr(2)}o.sort()}i=o;a=i.length;b=0;for(var e=a-1,c;e-b>1;){c=b+(e-b>>1);if(i[c]<=m)b=c;else e=c}if(i[b].indexOf(m)===0)b=b;else if(i[e].indexOf(m)===0)b=e;else return[];for(a=b+21>a?a-1:b+20;i[a].indexOf(m)!==0;)a--;return i.slice(b,a)}})(window,window["10kse"]);
(function(p,r,k){function o(e){function c(){if(l){var j=l.text(),q=f.val(),t=q.lastIndexOf(" ")+1;f.get(0).value=s=q.substr(0,t)+j}d.hide()}function g(){var j=f.offset();d.css({top:j.top+e.offsetHeight,left:j.left,width:e.offsetWidth});d.show()}var f=k(e),d=k('<div class="auto"><ul></ul></div>').css({display:"none"}).appendTo(document.body),n=d.find("ul"),s="",l=null;f.bind("change keyup",function(j){var q=f.val();j=j.which;if(q!==s){s=q;j=q.lastIndexOf(" ");q=q.substr(j<0?0:j).toLowerCase();if(q.length)if((j=
b(q))&&j.length){n.empty();l=null;for(var t=0,v=j.length;t<v;t++)k("<li><span></span></li>").appendTo(n).find("span").html(j[t].split(q,2).join("<b>"+q+"</b>"));g()}else d.hide();else d.hide()}else if(j===h||j===m){if(l)if(j===h){q=l.prev();if(q.length){l.removeClass(a);l=q.addClass(a)}}else{q=l.next();if(q.length){l.removeClass(a);l=q.addClass(a)}}else l=n.find("li").eq(0).addClass(a);g()}else j===i&&c()}).parent("form").submit(c);n.mouseover(function(j){j=k(j.target);j=j.is("li")?j:j.parent("li");
if(j=j.length?j:null){l&&l.get(0)!==j.get(0)&&l.removeClass(a);l=j.addClass(a)}}).click(c)}var h=38,m=40,i=13,a="selected",b=r.suggest;k(".search input").each(function(){o(this)});r.autocomplete=o})(window,window["10kse"],jQuery);
